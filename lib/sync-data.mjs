...
const comparePlayIds = new Intl.Collator('en', { numeric: true }).compare;
...
export function normalizePlays(data, now = Date.now()) {
  ...
  const ordered = [...plays.values()].sort((a,b) => comparePlayIds(a.id, b.id));
  return { checkedAt: now, plays: ordered, conflict: ordered.some((p,i) => i > 0 && p.utc < ordered[i-1].utc) };
}
export async function syncData(pathname, {fetcher = fetch, signal, now = Date.now} = {}) {
  if (pathname === '/api/sync/teams') {
    const data = await readJSON(`${base}/teams?limit=1000`, {fetcher, signal});
  ...
  if (schedule) return normalizeSchedule(await readJSON(`${base}/teams/${schedule[1]}/schedule?season=${schedule[2]}`, {fetcher, signal}));
  const plays = /^\/api\/sync\/plays\/(\d{1,12})$/.exec(pathname);
  if (plays) return normalizePlays(await readJSON(`${base}/summary?event=${plays[1]}`, {fetcher, signal}), now());
  return null;
}
