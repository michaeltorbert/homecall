import { sealMediaTarget, mediaKeyConfigured } from './media-token.mjs';
...
export async function relayMedia(request, target, { fetcher = fetch, secret, sourceId, version, now = Date.now(), gatewayOrigin = new URL(request.url).origin, headerTimeoutMs = 10000, bodyIdleTimeoutMs = 30000, nativeBody = false } = {}) {
