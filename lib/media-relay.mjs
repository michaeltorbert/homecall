import { sealMediaTarget } from './media-token.mjs';
...
async function rewritePlaylist(text, base, target, options) {
  ...
  const rewrite = async (value, kind = 'resource') => {
    ...
    const child = { ...target, url: new URL(value, base).href, kind };
    validateMediaTarget(child);
    const cacheKey = `${kind}:${child.url}`;
    if (resources.has(cacheKey)) return resources.get(cacheKey);
    const token = await sealMediaTarget({ target: { url: child.url, kind: child.kind, allowedOrigins: [new URL(child.url).origin] }, sourceId: options.sourceId, version: options.version }, options.secret, { now: options.now });
    const resource = `${options.gatewayOrigin}/media/resource/${token}`;
    resources.set(cacheKey, resource);
    return resource;
  };
  const output = [];
  for (const line of lines) {
    if (line.length > 8192 || /[\x00-\x08\x0b-\x1f]/.test(line)) throw new Error('Invalid playlist');
    ...
    if (URI_TAGS.has(tag)) {
      ...
      const scrubbed = line.replace(/(?:[:,])URI="[^"\r\n]+"/, '');
      if (/(?:https?:|\/\/|(?:URL|URI)\s*=)/i.test(scrubbed)) throw new Error('Unsupported URI attribute');
      output.push(rewritten); continue;
    }
    if (tag === '#EXT-X-STREAM-INF') { if (/(?:https?:|\/\/|URI\s*=)/i.test(line)) throw new Error('Unsupported URI'); nextPlaylist = true; output.push(line); continue; }
    if (!SIMPLE.has(tag) || /(?:https?:|\/\/|(?:URL|URI)\s*=)/i.test(line)) throw new Error('Unsupported playlist tag');
...
export async function relayMedia(request, target, { fetcher = fetch, secret, sourceId, version, now = Date.now(), gatewayOrigin = new URL(request.url).origin, headerTimeoutMs = 10000, bodyIdleTimeoutMs = 30000 } = {}) {
  ...
  try {
    ...
    if (upstream.status === 404) { await upstream.body?.cancel(); cleanup(); return failure(404); }
    ...
    if (target.kind === 'key') {
      ...
      if (request.method === 'HEAD') { await upstream.body?.cancel(); cleanup(); return new Response(null, { headers: outputHeaders }); }
    ...
    if (isPlaylist) {
      ...
      const timer = setTimeout(abort, headerTimeoutMs);
      let rewritten;
      try { rewritten = await rewritePlaylist(await boundedText(upstream.body, controller.signal), url, target, { secret, sourceId, version, now, gatewayOrigin }); } finally { clearTimeout(timer); }
