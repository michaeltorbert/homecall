...
function validDirectory(target, directory) {
  try { validateMediaTarget({ ...target, url: directory, kind: 'resource' }); return true; } catch { return false; }
}
...
async function rewritePlaylist(text, base, target, options) {
  ...
  const seal = async (url, kind, scope) => sealMediaTarget({ target: { url, kind, allowedOrigins: [new URL(url).origin], ...(scope ? { scope } : {}) }, sourceId: options.sourceId, version: options.version }, options.secret, { now: options.now });
...
export async function relayMedia(request, target, { fetcher = fetch, secret, sourceId, version, now = Date.now(), gatewayOrigin = new URL(request.url).origin, headerTimeoutMs = 10000, bodyIdleTimeoutMs = 30000, nativeBody = false, directory = null } = {}) {
  ...
  try {
    ...
    if (isPlaylist) {
      ...
      const timer = setTimeout(abort, headerTimeoutMs);
      let rewritten;
      try { rewritten = await rewritePlaylist(await boundedText(upstream.body, controller.signal), url, target, { secret, sourceId, version, now, gatewayOrigin, directory }); } finally { clearTimeout(timer); }
