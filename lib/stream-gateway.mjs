...
export async function streamGateway(request,env,{origins=[],fetcher=fetch,target=request.url.replace(/^https?:\/\/[^/]+/,''),nativeBody=false}={}) {
  ...
  try {
    ...
    if(matchGame){
      ...
      if(form){
        const token=await sealMediaTarget({target:{url:form.directory,kind:'resource',allowedOrigins:[new URL(form.directory).origin],scope:'directory'},sourceId,version:catalog.version},env.MEDIA_TOKEN_KEY);
        headers.set('Content-Type','application/vnd.apple.mpegurl');
        return new Response(request.method==='HEAD'?null:`#EXTM3U\n#EXT-X-VERSION:3\n#EXT-X-STREAM-INF:BANDWIDTH=128000\n${gatewayOrigin}/media/resource/${token}/${form.name}\n`,{headers});
      }
    }
    const response=await relayMedia(request,media,{fetcher,secret:env.MEDIA_TOKEN_KEY,sourceId,version:catalog.version,gatewayOrigin,nativeBody,directory});
