...
import { openMediaTarget } from './media-token.mjs';
...
export async function streamGateway(request,env,{origins=[],fetcher=fetch,target=request.url.replace(/^https?:\/\/[^/]+/,'')}={}) {
  ...
  try {
    ...
    const response=await relayMedia(request,media,{fetcher,secret:env.MEDIA_TOKEN_KEY,sourceId,version:catalog.version,gatewayOrigin});
