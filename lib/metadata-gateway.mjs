...
export async function metadataGateway(request, { target = request.url.replace(/^https?:\/\/[^/]+/, ''), method = request.method, origins = [PRODUCTION_ORIGIN], fetcher = fetch, cache, ctx, now = Date.now } = {}) {
  ...
  const key = new Request(new URL(`/__metadata_cache_v1${target}`, request.url));
  let entry;
  if (cache) {
    try {
  ...
  try {
    if (!entry) {
      const data = await adapter(target, { fetcher, signal: request.signal, now });
    ...
    if (target.startsWith('/api/sync/plays/')) {
      const ageMs = Math.ceil(now() - entry.data.checkedAt);
      if (!Number.isSafeInteger(ageMs) || ageMs < 0) throw Error('timing-age-unavailable');
