...
export async function metadataGateway(request, { target = request.url.replace(/^https?:\/\/[^/]+/, ''), method = request.method, origins = [PRODUCTION_ORIGIN], fetcher = fetch, cache, ctx, now = Date.now, diagnostic = () => {} } = {}) {
  ...
  try {
    if (!entry) {
      const data = await adapter(target, { fetcher, signal: request.signal, now, diagnostic });
