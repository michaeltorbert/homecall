...
export async function readPrivateCatalog(env) {
  ...
  if (typeof raw !== 'string' || new TextEncoder().encode(raw).length > MAX_BYTES) throw Error('Private catalog unavailable');
  try { return validatePrivateCatalog(JSON.parse(raw)); } catch { throw Error('Private catalog invalid'); }
}
...
export function findArchiveTarget(catalog, school, id) {
  ...
  if (!item) return null;
  return {url:item.url,allowedOrigins:[new URL(item.url).origin],kind:'audio'};
}
