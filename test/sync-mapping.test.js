...
import { clockSeconds,footballSeason,matchEvent,availableAnchors,selectAnchors,gameOrder } from '../src/sync-mapping.js';
import { normalizeSchedule,normalizePlays,syncData } from '../lib/sync-data.mjs';
const base=Date.parse('2026-09-12T23:00:00Z');
const header={id:'123',uid:'s:20~l:23~e:123',season:{year:2026},league:{id:'23',slug:'college-football'},competitions:[{id:'123',competitors:[{team:{id:'150'}},{team:{id:'356'}}]}]};
...
test('event matching requires both schools, date proximity and a unique result',()=>{
 const event={id:'123',start:base,season:2026,teams:['Georgia Tech','Tennessee']},game={start:base,opponent:'Tennessee'};
...
test('schedule normalization and gateway reject arbitrary hosts or malformed routes',async()=>{
 assert.equal(normalizeSchedule({team:{id:'150'},season:{year:2026},events:[{id:'123',season:{year:2026},date:new Date(base).toISOString(),competitions:[{id:'123',competitors:[{team:{id:'150',location:'Duke'}},{team:{id:'356',location:'Illinois'}}]}]}]})[0].id,'123');
...
test('verified Southern Mississippi alias matches Auburn without fuzzy opponent matching',()=>{
 const event={id:'401856671',start:base,season:2026,teams:['Auburn','Southern Miss']};
...
test('play ordering preserves provider encounter order without assuming IDs encode chronology',()=>{
 ...
 assert.deepEqual(data.plays.map(play=>play.id),ids);
 assert.equal(data.conflict,true);
});


test('January football belongs to prior season and event identity rejects wrong team or season',()=>{
 const january=Date.parse('2027-01-02T23:00:00Z');assert.equal(footballSeason(january),2026);
 const game={start:january,opponent:'Illinois'},event={id:'123',start:january,season:2026,teams:['Duke','Illinois'],teamIds:['150','356']};
 assert.equal(matchEvent([event],'Duke',game,'150'),event);
 assert.equal(matchEvent([{...event,season:2027}],'Duke',game,'150'),null);
 assert.equal(matchEvent([event],'Duke',game,'59'),null);
 assert.equal(selectAnchors([{quarter:5,clock:'1:00'}],5,'1:00').status,'invalid');
});
