...
Miami's official radio affiliation points to WQAM/Audacy; direct browser-playable game audio is still being verified. Require a public official-origin/config-derived endpoint with appropriate CORS and no bypass of station rights restrictions. If no usable direct stream exists, show the official listening destination and an explicit unsupported in-app source state; do not claim Miami alignment works merely because a team button exists. Treat Miami direct playback as an acceptance gap requiring continued investigation.
