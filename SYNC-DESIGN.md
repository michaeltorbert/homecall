...
The media element feeds the worklet through Web Audio. Known `waiting`/ended/error states gate incoming samples without treating legitimate silence as a gap. A network `stalled` notification gates only when the element lacks future data. These events are best-effort browser signals: they cannot expose every broadcaster ad splice or prove continuous original program content. Context interruptions invalidate listener confirmation and cancel any hold. Reconnect/team change creates a new context/engine epoch and discards all PCM; stale async callbacks and acknowledgments cannot update the replacement player. A user-command timeout closes the entire epoch and disconnects, preventing an unacknowledged queued change from applying to continuing audio later. Idempotent internal ingest/interruption controls retain at most 64 pending acknowledgments and may wait through context suspension without disconnecting. Exceeding that bound explicitly disconnects rather than growing memory indefinitely. Startup stays bounded even if its initial control cannot be acknowledged. A native source pause holds output as well as gating input, while network buffering may drain retained audio. Resume has a bounded deadline too; failed recovery disconnects.
